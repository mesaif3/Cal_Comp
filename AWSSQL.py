...
# db.execute("CREATE TABLE sessions (session_id INTEGER NOT NULL IDENTITY(1,1), session_name VARCHAR(20) NOT NULL, PRIMARY KEY (session_id))")

# db.execute("CREATE TABLE users (user_id INTEGER NOT NULL IDENTITY(1,1), user_name VARCHAR(20) NOT NULL, user_schedule TEXT NOT NULL, user_color VARCHAR(20), PRIMARY KEY (user_id))")

# db.execute("CREATE TABLE session_users (session_id INTEGER REFERENCES sessions(session_id) ON UPDATE CASCADE ON DELETE CASCADE, user_id INTEGER REFERENCES users(user_id) ON UPDATE CASCADE ON DELETE CASCADE, user_name VARCHAR(20) NOT NULL, PRIMARY KEY (session_id, user_id))")
...
sess_id = {"0": {"session_name": "saif", "people": [saif]},
        ...
        "1241": {"session_name": "The Jungle", "people": []}}

db.execute("SET IDENTITY_INSERT sessions ON")
for id in list(sess_id.keys()):
    db.execute("INSERT INTO sessions (session_id, session_name) VALUES(?,?)", id, sess_id[id]['session_name'])

db.execute("SET IDENTITY_INSERT sessions OFF")

db.execute("SET IDENTITY_INSERT users ON")
for id in ['2']:
